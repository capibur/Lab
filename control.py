...
lambda0 = 6e-6  # центральная длина волны, м
w0 = 2 * np.pi * c / lambda0
lambda_phases = 6.20e-7
p_sat = 0.0276
file_names = os.listdir('2026.03.19')
for i in file_names:
    filename = f'2026.03.19/{i}'
    ...
    ax1.plot(delay, signal)
    ax2.plot(freq[mask_freq], spectrum_norm, label="Экспериментальный спектр")
    ax2.plot(freq_small, spec_model_on_freq, label="Теоретический спектр (с поглощением)")
    ax2.set_xlim(0.30e14, 0.65e14)
    ax2.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'grap/{i[0:-4]}.jpg')
    plt.close()
